  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        if (normalizedError.includes("configured")) {
          setError(
            "Auth is not configured yet. Set MONGODB_URI and NEXTAUTH_SECRET in your environment.",
          );
          return;
        }
      ...
      <section className="relative hidden lg:flex items-center justify-center overflow-hidden bg-slate-900">
        ...
        <div className="relative max-w-md p-10 text-white">
          <h1 className="text-4xl font-black tracking-tight sm:text-5xl">
            Control Every Dollar With Clarity
          </h1>
      ...
      <section className="flex items-center justify-center bg-white px-4 py-12 sm:px-10">
        <div className="w-full max-w-md space-y-8">
          <div className="space-y-3">
            ...
            <p className="text-base text-slate-600">Log in to access all of our content</p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between text-sm text-slate-600">
              <span className="cursor-pointer font-medium text-sky-700 hover:text-sky-900">
                Forgot password
              </span>
            </div>
